...
import io.kudos.ai.ability.model.audio.support.enums.impl.AudioResponseFormatEnum
...
class TTSModelTest {
    ...
    fun test_generate_speech_with_prompt() {
        ...
        val opts = buildSpeechOptions(TTSModelEnum.KOKORO_82M)
    ...
    fun test_generate_speech_chinese_text() {
        ...
        // val voice = ttsModel.getDefaultChineseVoice()
        
        val opts = buildSpeechOptions(TTSModelEnum.PIPER_ZH_CN_HUAYAN_MEDIUM)
    ...
    private fun buildSpeechOptions(
        ttsModel: TTSModelEnum,
        responseFormat: AudioResponseFormatEnum = AudioResponseFormatEnum.MP3,
        speed: Double = 1.0,
        voice: IVoiceEnum? = null
    ): OpenAiAudioSpeechOptions {
        val modelId = ttsModel.modelName
        val voiceId = if (voice != null) {
            // 如果传入了 voice 枚举，优先使用它
            log.debug("Using provided voice enum: ${voice.voiceId} (${voice.displayName ?: "N/A"}) for model: $modelId")
            voice.voiceId
        } else {
            // 如果没有传入，尝试从模型枚举获取推荐的 voice
            val defaultVoice = ttsModel.getDefaultVoice()
    ...
    companion object {
        ...
        fun registerProps(registry: DynamicPropertyRegistry) {
            ...
            SpeachesTestContainer.startIfNeeded(registry, listOf(
                TTSModelEnum.KOKORO_82M.modelName,
                TTSModelEnum.PIPER_ZH_CN_HUAYAN_MEDIUM.modelName,
            ))
